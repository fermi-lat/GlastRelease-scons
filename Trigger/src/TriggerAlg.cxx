...
StatusCode TriggerAlg::execute() 
{
    ...
    */
    ...
    if(m_firstevent){
        // test for overlays... is there a more general way?
        SmartDataPtr<Event::TkrDigiCol> 
            overlayDigiCol(eventSvc(), EventModel::Overlay::TkrDigiCol);
        if(overlayDigiCol) { m_isOverlay = true;}
    }
    if(m_isOverlay && gem!=0) { tkrvector = gem->tkrVector(); }

    // get the livetime for the gem and the metadata
    ...
    if (m_isMc && (gem == 0 || m_isOverlay)){
        ...
        LdfEvent::GemOnePpsTime ppsTimeTds(m_LivetimeSvc->ticks(
            (unsigned int)now) & 0x1ffffff, ((unsigned int) now) & 0x7f);
