...
//
    ...
    // The following definitions to create a TDS object creates a map between clusters and energy
    typedef std::vector<Event::CalEventEnergy*>             CalEventEnergyVec;
    typedef std::map<Event::CalCluster*, CalEventEnergyVec> CalEventEnergyMapDef;

    // Define the TDS version of the map between trees and the relations
    static const CLID& CLID_CalEventEnergyMap  = InterfaceID("CalEventEnergyMap",  1, 0);

    class CalEventEnergyMap : public CalEventEnergyMapDef, public DataObject 
    {
        //! Retrieve pointer to class defininition structure
        virtual const CLID& clID() const   { return CalEventEnergyMap::classID(); }
        static const CLID& classID()       { return CLID_CalEventEnergyMap; }
    };
