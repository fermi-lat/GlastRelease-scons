...
// Externals 
extern const CLID& CLID_RefTable1to1;
extern const CLID& CLID_RefTable1toN;
...
public:
  ...
  */
  ...
  typedef TableType::container_type::iterator       iterator;
  /// Definition of map iterator (CONST)
  typedef TableType::container_type::const_iterator const_iterator;
#else
  /// Definition of map iterator
...
protected:
  ...
  EntryType* i_reference(const FROM* from)  {
    TableType::value_type* i = m_table.find( from );
    if ( i != 0 )    {
      return &((*i).second);
    }
    return 0; 
  ...
  const EntryType* i_reference(const FROM* from)  const  {
    const TableType::value_type* i = m_table.find( from );
    if ( i != m_table.end() )    {
      return &((*i).second);
    }
    return 0; 
