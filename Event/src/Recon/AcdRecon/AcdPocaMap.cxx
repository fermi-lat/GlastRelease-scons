...
void AcdPocaMap::add(const Event::AcdTkrPoca& poca) {

  int trackIdx = poca.trackIndex();
  const idents::AcdId& id = poca.acdId();
...
const Event::AcdTkrPoca* AcdPocaMap::getBestPoca(const idents::AcdId& acdId) const {
  std::map< idents::AcdId, Event::AcdPocaSet >::const_iterator it = m_tileToPocaMap.find(acdId);
...
const Event::AcdTkrPoca* AcdPocaMap::getBestPoca(int trackIdx) const {
  std::map< int, Event::AcdPocaSet >::const_iterator it = m_trackToPocaMap.find(trackIdx);
...
const Event::AcdTkrPoca* AcdPocaMap::getPoca(const idents::AcdId& acdId, int trackIdx ) const {
