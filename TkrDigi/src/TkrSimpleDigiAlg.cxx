...
private: 
	...
	*/ 
	...
	double m_totMax;
	/// maximum number of hits per controller (per plane)
	int    m_maxHits;
...
:Algorithm(name, pSvcLocator)
{
	...
	declareProperty("totAt1Mip"     , m_totAt1Mip     = 43.8  );
	...
	declareProperty("maxHitsPerSide", m_maxHits       = 63);
...
StatusCode TkrSimpleDigiAlg::execute()
{
	...
	for(SiPlaneMap::iterator si = m_SiMap.begin(); si != m_SiMap.end(); ++si){
		...
		if (size>m_maxHits) {
			...
			while (ihit<pDigi->getNumHits()) {
				...
				if (index>lastC0Strip) {
					...
					if (ihit >= size-m_maxHits) break; // were under the limit now, no more to remove
		...
		if (nStrips>1) {
			int ihit;
			std::cout << "Multiple strips: ";
			for (ihit=0;ihit<pDigi->getNumHits();ihit++) {
				std::cout << pDigi->getHit(ihit) << " " ;
			}
			std::cout << std::endl;
		}
        */
