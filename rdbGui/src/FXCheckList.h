...
* $Id$                         *
********************************************************************************/
#ifndef FXCHECKLIST_H
#define FXCHECKLIST_H
...
/// List styles
enum {
  CHECKLIST_EXTENDEDSELECT    = 0,             /// Extended selection mode allows for drag-selection of ranges of items
  CHECKLIST_SINGLESELECT      = 0x00100000,    /// Single selection mode allows up to one item to be selected
  CHECKLIST_BROWSESELECT      = 0x00200000,    /// Browse selection mode enforces one single item to be selected at all times
  CHECKLIST_MULTIPLESELECT    = 0x00300000,    /// Multiple selection mode is used for selection of individual items
  CHECKLIST_AUTOSELECT        = 0x00400000,    /// Automatically select under cursor
...
/// List item
class FXAPI FXCheckListItem : public FXObject {
  FXDECLARE(FXCheckListItem)
  friend class FXCheckList;
protected:
  FXString  label;
  FXIcon   *icon;
  void     *data;
...
protected:
  ...
  void drawFocus(const FXCheckList* list,FXDC& dc,FXint x,FXint y,FXint w,FXint h) const;
  virtual FXint hitItem(const FXCheckList* list,FXint x,FXint y) const;
protected:
  enum {
    SELECTED  = 1,
    FOCUS     = 2,
...
/// List Widget
class FXAPI FXCheckList : public FXScrollArea {
  FXDECLARE(FXCheckList)
protected:
  FXCheckListItem   **items;             // Item list
  ...
  FXCheckListSortFunc sortfunc;          // Item sort function
...
protected:
  ...
  virtual void layout();
  void recompute();
  virtual FXCheckListItem *createItem(const FXString& text,FXIcon* icon,void* ptr);
private:
  FXCheckList(const FXCheckList&);
  FXCheckList &operator=(const FXCheckList&);
public:
...
public:
  static FXint ascending(const FXCheckListItem* a,const FXCheckListItem* b);
  static FXint descending(const FXCheckListItem* a,const FXCheckListItem* b);
public:
  enum {
...
public:
  ...
  FXCheckList(FXComposite *p,FXint nvis,FXObject* tgt=NULL,FXSelector sel=0,FXuint opts=CHECKLIST_NORMAL,FXint x=0,FXint y=0,FXint w=0,FXint h=0);
  ...
  FXCheckListItem *retrieveItem(FXint index) const;
  ...
  */
  ...
  FXbool isItemChecked(FXint index) const;
  ...
  /// check item
  FXbool checkItem(FXint index,FXbool notify=FALSE);

  /// uncheck item
  FXbool uncheckItem(FXint index,FXbool notify=FALSE);

  /// Toggle item check state
  FXbool toggleCheckItem(FXint index,FXbool notify=FALSE);
  ...
  FXCheckListSortFunc getSortFunc() const { return sortfunc; }

  /// Change sort function
  void setSortFunc(FXCheckListSortFunc func){ sortfunc=func; }
