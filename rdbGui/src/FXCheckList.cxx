...
// Draw item
void FXCheckListItem::draw(const FXCheckList* list,FXDC& dc,FXint x,FXint y,FXint w,FXint h){
  FXint ih=0,th=0;
...
// Can have focus
FXbool FXCheckList::canFocus() const { return TRUE; }
...
long FXCheckList::onEnter(FXObject* sender,FXSelector sel,void* ptr){
  ...
  if(!timer){timer=getApp()->addTimeout(this,ID_TIPTIMER,getApp()->getMenuPause());}
  cursor=-1;
  return 1;
  }
...
FXint FXCheckList::insertItem(FXint index,FXCheckListItem* item,FXbool notify){
  ...
  memmove(&items[index+1],&items[index],sizeof(FXCheckListItem*)*(nitems-index));
  items[index]=item;
  nitems++;
  ...
  if(0<=current && current==index){
    ...
    if((options&SELECT_MASK)==CHECKLIST_BROWSESELECT && items[current]->isEnabled()){
      selectItem(current,notify);
      }
    }
...
// Insert item
FXint FXCheckList::insertItem(FXint index,const FXString& text,FXIcon *icon,void* ptr,FXbool notify){
...
// Append item
FXint FXCheckList::appendItem(const FXString& text,FXIcon *icon,void* ptr,FXbool notify){
...
// Prepend item
FXint FXCheckList::prependItem(const FXString& text,FXIcon *icon,void* ptr,FXbool notify){
...
void FXCheckList::removeItem(FXint index,FXbool notify){
  ...
  nitems--;
  delete items[index];
  memmove(&items[index],&items[index+1],sizeof(FXCheckListItem*)*(nitems-index));
...
// Get item by name
FXint FXCheckList::findItem(const FXString& text,FXint start,FXuint flags) const {
