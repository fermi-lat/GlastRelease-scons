...
int main(int argn, char** argc) {
  try {
    ...
    std::string dirname("$(EVENTRETROOT)/test/arch");
    unsigned int scid = 99;
    std::vector<eventRet::ApidSpan*> apIdCol;

    apIdCol.push_back(new eventRet::ApidSpan(955, 1137447000.0, 1137447160.0));
    apIdCol.push_back(new eventRet::ApidSpan(956, 1137447000.0, 1137447160.0));


    if( argn>1 ) dirname = argc[1];
    //if (argn>2 ) scid = argc[2];
    

    if (facilities::Util::expandEnvVar(&dirname) < 0) {
        std::cout << "Failed to expand env variable in directory name "
                  << dirname << std::endl;
        return -1;
    }
    ...
    std::cout << "Using directory: " << dirname << std::endl;

    DfiParser *ebfP = new DfiParser(dirname, scid, apIdCol);
    ...
    do {
        ...
        printf("Event Size in Bytes %llu \n", myLatData->eventSizeInBytes());
