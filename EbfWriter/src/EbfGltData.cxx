...
/*
 ...
 | the GLT's contribution to the event data.
 |
 ...
 | logic forming the ACD throttle is on the GLT not the ACD. The
 ...
 | are used are in the GLT. The CNO trigger is really a collection
 ...
 | of the GLT code to turn them into a trigger.
 |
 |
 | !!! IMPROVEMENT OPPORTUNITY !!!
 | -------------------------------
 |
 | TIME HANDLING and GLT Modeling
 | These registers and their underlying supporting counters are part
 | of the GLT. However, one could make a case that the should not
 ...
 | in the state that I received it did not model the GLT (it only
 ...
 | a more complete model of the GLT implemented in an architecturally
 | consistent way.
 |
*/ 
 ...
 *                     double               mc_time,
 *                     EbfGltCounters *lat_counters,
 *                     const EbfAcdData        *acd,
 *                     const EbfTkrData        *tkr,
 *                     const EbfCalData        *cal)
 *  @brief  Fills  the Glt Record and serves as the basis of the 
 *          trigger decision.
 *
 *  @param  mc_number  The identifying GLEAM event number. This
 *                     is squirreled away in the GLT so that it
 ...
 *                     and their supporting registers on the GLT.
 ...
 *                     the GLT information.
 */
void EbfGltData::fill (unsigned int       mc_number,
                       double               mc_time,
                       EbfGltCounters *lat_counters,
                       const EbfAcdData        *acd,
                       const EbfTkrData        *tkr,
                       const EbfCalData        *cal)
{
    ...
    /*
     ...
     |    0. Track 3-in-a-row                       (per tower)
     |    1. Throttled towers by the ACD            (per tower) 
     |    2. CAL HI                                 (per tower)
     |    3. CAL LO                                 (per tower)
     |    4. ACD CNO                                (global   )
     |
    */
    ...
    /* Copy the trigger information to the GLT */
    m_thrTkr    = (throttled << 16) | threeInARow;
    m_calHiLo   = (calHi     << 16) | calLo;
    m_cnoReqvec = (cno       << 16) | reqVec;


    /* Copy the ACD hit data from the ACD record to the GLT */
    m_tiles[XZ] = xz;
    m_tiles[YZ] = yz;
    m_tiles[XY] = xy;
    m_tiles[RU] = ru;
    ...
    /* 
     | Fill in the GLT busy, prescale and sent counters.
     |  
     | !!! KLUDGE !!!
     | --------------
    ...
    */
    m_busy       = lat_counters->busy ();
    m_prescaled  = lat_counters->prescale ();
    m_sent       = lat_counters->increment_sent ();


    /*
     | TIME REGISTER AND COUNTERS
     | This section fills in the GLT's time registers that eventually
     | go into the GLT's contribution to the event data. These are
     |
    ...
    */
    ...
    m_ppstime |= (whole_seconds & 0x3f) << 25;
...
/**
 *
 *  @fn     unsigned int *EbfGltData::format (unsigned int *dst)
 *  @brief  Converts the GLT data into Event Builder Format.
 *
 ...
 *
 */
unsigned int *EbfGltData::format (unsigned int *dst) const
{
   dst[0]  = m_thrTkr;
   dst[1]  = m_calHiLo;
   dst[2]  = m_cnoReqvec;
   ...
   /* 
    | This is a hack, for debugging purposes the MC event number is stuffed
    | into an unused location. The GLT record is always rounded to an
    | even number of LCB packets (4 32-bit words). The length of the GLT
    | is fixed and is precisely one 32 bit word short of the boundary. The
    | MC event number is stuffed in that location. (The GLT record count
    | includes the 2 word LCB header.)
   */
   dst[13] = m_number;
 ...
 * 
 *  Routine to print the GLT data for this event. This is mainly used
 *  used to debug the fill and format routines,
 *
 */
void EbfGltData::print() const
{
   std::printf ("GLT    ThrTkr = %8.8x\n"
                "      CalHiLo = %8.8x\n"
                "    CnoReqvec = %8.8x\n"
...
/*
   ...
   RETURNS
   -------
   The list of shadowed tiles. This list is right justified, with
...
*/
{
   ...
   while (tiles)
   {
       ...
       if (tiles < 0) throttle |= towers[itile];
