#ifndef EBF_GEMCOUNTERS_H
#define EBF_GEMCOUNTERS_H

#include <math.h>

/**
 *  @class EbfGemCounters
 *  @brief Models the various counters on the GEM that are sourced
 *         for event information.
 *
 *   This models the various counters on the GEM that are sourced for
 ...
 *
 *   TIME
 *   The GEM maintains 3 registers to keep track of the time
 *
...
class EbfGemCounters
{
  ...
  **/
   ...
   *
   *  @fn     unsigned int ticks (double time)
   *  @brief  Returns the difference in time in clock ticks
  **/
  inline unsigned int deltaTicks(double deltaTime) const
  {
      return (unsigned int)(m_frequency * deltaTime);

//      return (unsigned int)(m_frequency 
//                  * (deltaTime +  m_amplitude * cos (m_iperiod * deltaTime)));
  }
  ...
  **/
  ...
  inline EbfGemCounters (void )  {   return;  }
