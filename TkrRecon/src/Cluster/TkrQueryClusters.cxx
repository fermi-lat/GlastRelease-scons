...
Point TkrQueryClusters::meanHit(Event::TkrCluster::view v, int iplane)
{
    ...
    if (iplane < 0 || iplane > Event::TkrClusterCol::NPLANES-1) return Pini;
...
Point TkrQueryClusters::meanHitInside(Event::TkrCluster::view v, int iplane, double inDistance,
								 Point Pcenter)
{
    ...
    // Kludge to prevent crashes when requested plane out of range
    if (iplane < 0 || iplane > Event::TkrClusterCol::NPLANES-1) return P;
...
Point TkrQueryClusters::nearestHitOutside(Event::TkrCluster::view v, int iplane, 
									 double inDistance, Point Pcenter, int& id)
{
    ...
    // Kludge to prevent crashes when requested plane out of range
    if (iplane < 0 || iplane >= Event::TkrClusterCol::NPLANES-1) return Pnear;
...
int TkrQueryClusters::numberOfHitsNear( int iPlane, double dX, double dY, Point& x0)
{
    ...
    // Kludge to prevent crashes when requested plane out of range
    if (iPlane < 0 || iPlane > Event::TkrClusterCol::NPLANES-1) return numHits;
...
int TkrQueryClusters::numberOfHitsNear( Event::TkrCluster::view v, int iPlane, double inDistance, Point& x0)
{
