...
public :
    ...
    /// vector of (run,event) pairs
    runEventVector  m_runEvents;
    ...
    /// fill vector of (run,events) to prune
    void SetFileRunEvent(std::string fName) {

      int run, event;
        std::ifstream runEventFile;
       runEventFile.open(fName.c_str());

       while(!runEventFile.eof()) {
         runEventFile >> run; runEventFile >> event;
	 std::pair<int,int> re;
	 re = std::pair<int,int>(run,event);
	 m_runEvents.push_back(re);
       } 

       std::cout << " ---- List of Run, Event ids ----" << std::endl;
    ...
    void Peel(Long64_t numEvents=100000000); 
