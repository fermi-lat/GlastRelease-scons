#include "./src/pruneTrees/PruneRunEvent.h"
#include "TSystem.h"
#include <iostream>
#include <string>
#include <stdio.h>
...
text file containing list of run number/event ids for peeling
*/
int main(int argn, char** argc) {
  ...
  std::cout << "Chain Pruner with " << argn << " args " << std::endl; 
    ...
    if (argn > 4) {
      runEventFileStr = std::string(argc[4]);
    }

    if (argn > 5) {
        anaTupFileName = std::string(argc[5]) + "*";
    ...
    r.SetFileRunEvent(runEventFileStr);
    r.CopyTrees();
    r.Peel();
