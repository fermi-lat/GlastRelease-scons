#ifndef PruneTree_cxx
#define PruneTree_cxx 1
...
void RootTreeAnalysis::ReconTkr() {
    ...
    int numTracks = tkrRec->getTrackCol()->GetEntries();
    ((TH1F*)GetObjectPtr("TKRNUMFITTRACKS"))->Fill(numTracks);
    if (numTracks== 0) pruneEvent = false;
...
void RootTreeAnalysis::ReconAcd() {
    ...
    float activeDistance = acdRec->getActiveDist();
if (activeDistance > -20) pruneEvent = false;
    ((TH1F*)GetObjectPtr("ACDACTDIST"))->Fill(activeDistance);
...
{    
    ...
    if (anaTupTree) {
        anaTupTree->SetBranchStatus("*",1);  // disable all branches
        // activate desired branches
    ...
    for (Long64_t ievent=m_StartEvent; ievent<nMax; ievent++, curI=ievent) {
      ...
      if (pruneEvent) {
        if ((reconEventId != digiEventId) || (mcEventId != digiEventId)) {
          std::cout << "Event Ids do not match" << std::endl;
        }
        if ((reconRunNum != digiRunNum) || (mcRunNum != digiRunNum)) {
          std::cout << "Run Ids do not match" << std::endl;
        }
	CopyEntry(ievent);
	pruned++;
      }
 ...
 UInt_t RootTreeAnalysis::CopyTrees() {
    // Purpose and Method:  prepare copies of the trees
    //    We could be processing single files or chains, 
	//    This routine handles both casees.
    ...
    if (treeArr) {
	  ...
	  TFile* newFile = new TFile(fNameP.c_str(),"recreate");
	  fileArrCopy->Add(newFile);
	  TTree* newTree = ((TTree*)(treeArr->At(i)));
	  TTree* newTreeC = newTree->CloneTree(0);
	  treeArrCopy->Add(newTreeC);
        }
        return nb;
    }
    ...
    if (chainArr) {
	  ...
	  TFile* newFile = new TFile(fNameP.c_str(),"recreate");
	  fileArrCopy->Add(newFile);
	  TTree* newTree = ((TTree*)(chainArr->At(i)));
	  TTree* newTreeC = newTree->CloneTree(0);
	  chainArrCopy->Add(newTreeC);
        }
        return nb;
    }
