...
// Perugia method to retreive the Xvalue of an histogram giving to it
// a y value
   ...
   PsfModel(const std::string &outputFile, int maxTrys=5) 
      : Fitter(outputFile), m_maxTrys(maxTrys) {
        ...
        m_gauss = new TF1("myGaussModel", "x*([0]*exp(-0.5*x*x/([1]*[1])))");
        m_expo = new TF1("myExpoModel", "x*([2]*exp(-(x*[3])^[4]))");
        m_stgaus = new TF1("myStdGauss", "gaus");
...
// Total fit
      m_func->SetParameters(par0gaus[0], par0gaus[1]*2.,
                            par0expo[0], par0expo[1], par0expo[2]);
...
//    psf_thin.set_ymax(0.2);
//    psf_thin.set_ymin(1e-4);
...
//    psf_thick.set_ymax(0.5);
//    psf_thick.set_ymin(1e-4);
...
void fit_psfs() {
   std::string energyScaling("[0]*pow((1./McEnergy + 1./[2]), [1])");
//    std::string psf_model
//       = "[0]*(1. - 1./[1])*pow((1 + (pow(10., 2*x)/[2]/[2])/2./[1]), -[1])";
   std::string psf_model = "[0]/[1]*exp(-(x - [2])*(x - [2])/2./[1]/[1])";
   double lower_bounds[] = {1e-3, 0.2, -1.5};
   std::vector<double> lowerBounds(lower_bounds, lower_bounds + 3);
   double upper_bounds[] = {5e-2, 1., 1.5};
   std::vector<double> upperBounds(upper_bounds, upper_bounds + 3);
   double psf_params[] = {0.01, 0.4, -0.3};
   std::vector<double> psfParams(psf_params, psf_params+2);
   std::vector<double> params(3);
   Fitter * myfit;

// //   myfit = new PsfModel("my_psf_thin_parameters.root");
// //   double psf_params[] = {1., 2., 2.};
//    myfit = new GenericFitter(psf_model, psfParams, 
//                                       "my_psf_thin_parameters.root");
//    myfit->setBounds(lowerBounds, upperBounds);
//    MakeDists my_psf_thin("my_psf_thin.root", false, true);
...
//       my_psf_thin.project("BestDirErr", -1.5, 1.5, 100);
//    }
...
//                                 "energyScaling");
   ...
   psfParams[2] = -0.3;
   myfit = new GenericFitter(psf_model, psfParams, 
                             "my_psf_thick_parameters.root");
   myfit->setBounds(lowerBounds, upperBounds);
   MakeDists my_psf_thick("my_psf_thick.root", false, true);
...
//      my_psf_thick.project("BestDirErr", 0, 10, 100);
      my_psf_thick.project("BestDirErr", -1.5, 1.5, 100);
   }
