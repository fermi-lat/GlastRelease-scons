...
/** @class MIPFilterTool
    ...
    @author Tracy Usher
*/
class MIPFilterTool : public AlgTool, virtual public IFilterTool
{
public:

    // Standard Gaudi Tool constructor
    MIPFilterTool(const std::string& type, const std::string& name, const IInterface* parent);
...
//------------------------------------------------------------------------
MIPFilterTool::~MIPFilterTool()
{
}

StatusCode MIPFilterTool::initialize()
{
...
StatusCode MIPFilterTool::finalize ()
{
...
// Set Mode and Configuration for a given filter
void MIPFilterTool::setModeAndConfig(unsigned int mode, unsigned int config)
{
    // Get ObfInterface pointer
    ObfInterface* obf = ObfInterface::instance();
...
void MIPFilterTool::dumpConfiguration()
{
    MsgStream log(msgSvc(), name());

    log << MSG::INFO << "MIP Filter Setup - FSW release " << m_filterLibs->FlightSoftwareRelease() << "\n"
...
// This for end of run processing
void MIPFilterTool::eorProcessing()
{
    MsgStream log(msgSvc(), name());
