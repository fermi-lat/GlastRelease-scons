#ifndef SplusScriptEngineNode_h
#define SplusScriptEngineNode_h
...
/** @class SplusScriptEngineNode
*  @brief  Describes an ActivityNode encountered in reading an IM xml file
*
*/

class SplusScriptEngineNode : public IImActivityNode
{
public:
    typedef std::vector<std::string> StringList;

    SplusScriptEngineNode(const std::string& type, const std::string& name, const std::string& id) :
                     m_type(type), m_name(name), m_id(id) {m_nodeMap.clear();}
    virtual ~SplusScriptEngineNode() {}
...
#endif // ifdef SplusScriptEngineNode_h
