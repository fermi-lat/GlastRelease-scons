#ifndef SplusGraphEngineNode_h
#define SplusGraphEngineNode_h
...
/** @class SplusGraphEngineNode
*  @brief  Describes an ActivityNode encountered in reading an IM xml file
*
*/

class SplusGraphEngineNode : public IImActivityNode
{
public:
    typedef std::vector<std::string> StringList;

    SplusGraphEngineNode(const std::string& type, const std::string& name, const std::string& id) :
                     m_type(type), m_name(name), m_id(id) {m_nodeMap.clear();}
    virtual ~SplusGraphEngineNode() {}
...
#endif // ifdef SplusGraphEngineNode_h
