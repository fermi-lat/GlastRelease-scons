...
public:
    ...
    */
    ...
    public:
        ...
        virtual const int operator()(const std::string& name)=0;
    };
    ...
    */
    DecisionTreeBuilder(const DOMDocument* document, ILookUpIndex& searcher, std::ostream& log=std::cout,  int iVerbosity=0);
    ~DecisionTreeBuilder();
...
private:
    ...
    void parseNode(DOMElement* xmlElement, DecisionTree* decisionTree, std::map<std::string, int>& varIndexMap);
    ...
    std::string getCTOutputName(DOMElement* xmlActivityNode);
